...
function renderCaixaHTML(cfg) {
  ...
  return `<div class="page-header"><div class="page-title">${cfg.nome}</div><div class="total-display-val" id="${c}-total-badge">R$ 0,00</div></div><div class="g2"><div><div class="card"><div class="card-head">Numerário Físico</div><input type="text" class="field-input math-input" id="${c}-abertura" placeholder="Abertura"><hr>${notasHTML}<input type="text" class="field-input math-input" id="${c}-moeda" placeholder="Moedas"></div></div><div><div class="card"><div class="card-head">Eletrônico</div><input type="text" class="field-input math-input" id="${c}-pix" placeholder="PIX"><input type="text" class="field-input math-input" id="${c}-debito" placeholder="Débito"><input type="text" class="field-input math-input" id="${c}-credito" placeholder="Crédito"><input type="text" class="field-input math-input" id="${c}-taxa" placeholder="Taxas"></div><div class="card"><div class="card-head">Depósitos</div><input type="text" class="field-input math-input" id="${c}-deposito" placeholder="Depósito"></div><div class="card"><div class="card-head">Sistema</div><input type="text" class="field-input math-input" id="${c}-imob" placeholder="Valor Sistema"><div id="${c}-diff-block" class="diff-block"><div id="${c}-diff-val"></div></div></div></div></div>`;
}
    ...
    if (snap.exists()) {
      ...
      db.ref('cartorios/' + currentCartorioId + '/config').on('value', (cSnap) => {
        if(cSnap.exists()) {
          ...
          if(!window._uiInitialized) { loadConfigAndBuildUI(); showProfileSelector(); }
        }
      });
    }
  } else {
  ...
  db.ref(`cartorios/${currentCartorioId}/caixas/${dateStr}`).on('value', (snap) => {
    ...
    if(data) {
       CAIXAS.forEach(cfg => {
         ...
         ['pix','debito','credito','taxa','deposito','imob'].forEach(f => document.getElementById(`${c}-${f}`).value = data[c][f] || '');
         calcularCaixa(c);
       });
    }
...
function showProfileSelector() {
  ...
  list.innerHTML = OPERADORES.map(op => `<button onclick="setActiveOperator('${op.id}')">${op.nome}</button>`).join('');
}

function setActiveOperator(id) {
  activeOperator = OPERADORES.find(x => x.id === id);
  document.getElementById('profileModal').style.display = 'none';
  document.getElementById('active-operator-display').textContent = `Operador: ${activeOperator.nome}`;
  carregarDados();
}
